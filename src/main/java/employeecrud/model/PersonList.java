...
public class PersonList extends ArrayList<IPerson>{
	...
	public IPerson set(int index, IPerson element) {	
		...
		if(element.getId() == currentElement.getId() ||				
				!this.PersonWithIdExists(element.getId())) {
			
			return super.set(index, element);			
		}else{
			
			throw new IllegalArgumentException("There is already a Person with the same ID in this list");
	...
	private IPerson findPersonById(int id) {
		...
		List<IPerson> matches = this.stream()
			.filter(p -> p.getId() == id)
